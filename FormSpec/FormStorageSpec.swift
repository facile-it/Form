...
class FormStorageSpec: XCTestCase {
	...
	func testAllKeys() {
		property("'allKeys' is consistent with stored field values") <- forAll { (ap1: ArbitraryPair<Optional<ArbitraryFieldValue>,FieldKey>, ap2: ArbitraryPair<Optional<ArbitraryFieldValue>,FieldKey>, ap3: ArbitraryPair<Optional<ArbitraryFieldValue>,FieldKey>, ap4: ArbitraryPair<Optional<ArbitraryFieldValue>,FieldKey>) in

			let storage = FormStorage()

			let array = [ap1, ap2, ap3, ap4]
				.map { ($0.left?.get, $0.right) }
	...
	func testSetGetValue() {
		...
		property("'setValue' and 'getValue' should be consistent") <- forAll { (av: Optional<ArbitraryFieldValue>, ak: FieldKey) in
			let optValue = av?.get as FieldValue?
    ...
    func testSetGetOptions() {
        ...
        property("'setOptions' and 'getOptions' should be consistent") <- forAll { (ao: Optional<ArbitraryFieldValue>, av: Optional<ArbitraryFieldValue>, ak: FieldKey) in
            storage.set(value: av?.get as FieldValue?, at: ak)
            storage.set(options: ao?.get, at: ak)
            let optGotOption = storage.getOptions(at: ak)
            if ao == nil && optGotOption == nil { return true }
            guard let option = ao?.get, let gotOption = optGotOption else { return false }
    ...
    func testSetGetHidden() {
        ...
        property("'setHidden' and 'getHidden' should be consistent") <- forAll { (ab: Bool, av: Optional<ArbitraryFieldValue>, ak: FieldKey) in
            storage.set(value: av?.get as FieldValue?, at: ak)
